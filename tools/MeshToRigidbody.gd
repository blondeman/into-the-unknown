...
func _convert_to_rigidbody(mesh_instance: MeshInstance3D, parent: Node) -> void:
	...
	if BOX:
		...
		# Offset the collision shape so its centre matches the mesh AABB centre,
		# then apply the mesh's own local transform on top.
		var center_offset = Transform3D(Basis.IDENTITY, aabb.get_center())
		collision_shape.transform = mesh_transform * center_offset
	else:
